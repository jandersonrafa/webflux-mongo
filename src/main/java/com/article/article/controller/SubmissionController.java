...
public class SubmissionController {
	...
	public Mono<ModelAndView> pageDetail(@PathVariable("id") String articleId) {
		return articleService.findById(articleId).map(submission -> new ModelAndView("submission/submissionDetail", "submission", submission));
	}
	...
	public ModelAndView pageSubmission(@PathVariable("idEvento") String idEvento) {
		final SubmissionDetailDto dto = new SubmissionDetailDto();
		dto.setEvent(Event.builder().eventId(idEvento).build());
		return new ModelAndView("submission/submissionDetail", "submission", dto);
	}
}
